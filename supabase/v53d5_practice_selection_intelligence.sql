...
-- Production migration: 20260831073254 v53d5_practice_selection_intelligence
-- Adds per-topic learning evidence to the versioned ordinary-Practice retrieval
-- contract. V5.3D3 remains available as the rollback path.
...
begin
  ...
  topic_performance as (
    select
      lower(trim(coalesce(sa.strand,''))) as strand_key,
      lower(trim(coalesce(sa.topic,''))) as topic_key,
      count(*) filter (
        where sa.marks_awarded is not null
          and coalesce(sa.marks_possible,0) > 0
      )::integer as scored_responses,
      case
        when coalesce(sum(sa.marks_possible) filter (
          where sa.marks_awarded is not null
            and coalesce(sa.marks_possible,0) > 0
        ),0) > 0
        then round(
          ...
          / nullif(sum(sa.marks_possible) filter (
            where sa.marks_awarded is not null
              and coalesce(sa.marks_possible,0) > 0
          ),0)
        )::integer
    ...
    where v_ticket.roster_student_id is not null
      and ps.roster_student_id = v_ticket.roster_student_id
      and nullif(trim(coalesce(sa.topic,'')),'') is not null
    group by
      lower(trim(coalesce(sa.strand,''))),
      lower(trim(coalesce(sa.topic,'')))
  )
  select jsonb_build_object(
    ...
    'response_config',case coalesce(q.response_type,'text')
      ...
      when 'fraction' then jsonb_build_object(
        'simplest_form',coalesce((q.response_config->>'simplest_form')::boolean,false))
      when 'multi_blank' then jsonb_build_object('blanks',coalesce((
        ...
        from jsonb_array_elements(coalesce(q.response_config->'blanks','[]'::jsonb))
          with ordinality x(b,n)
      ),'[]'::jsonb))
      when 'multiple_choice' then jsonb_build_object(
        'options',coalesce(q.response_config->'options','[]'::jsonb))
      when 'multi_select' then jsonb_build_object(
        'options',coalesce(q.response_config->'options','[]'::jsonb))
      when 'drawing' then jsonb_build_object(
        'instructions',coalesce(q.response_config->>'instructions',''))
      when 'manual' then jsonb_build_object(
        'instructions',coalesce(q.response_config->>'instructions',''))
      else '{}'::jsonb end,
  ...
  left join question_history h on h.question_id = q.id
  left join topic_performance tp
    on tp.strand_key = lower(trim(coalesce(q.strand,'')))
   and tp.topic_key = lower(trim(coalesce(q.topic,'')))
...
comment on function public.get_student_practice_questions_v53d5(text,smallint) is
  'V5.3D5 ordinary-Practice retrieval with repeat history plus non-sensitive per-topic learning evidence for Mixed Practice selection intelligence. Eligibility and grading remain unchanged.';
