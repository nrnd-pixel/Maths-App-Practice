...
create policy "teachers read practice assignment attempts"
  on public.practice_assignment_attempts for select to authenticated
...
begin
  ...
  if v_attempt.id is not null and v_attempt.status='completed' then
    return jsonb_build_object('already_completed',true,'attempt_id',v_attempt.id,'assignment_id',v_assignment.id,
      'strand',v_assignment.strand,'topic',v_assignment.topic,'recommended_count',v_count);
  end if;

  if v_attempt.id is null then
    insert into public.practice_assignment_attempts(assignment_id,roster_student_id,status,started_at,updated_at)
    values(v_assignment.id,v_ticket.roster_student_id,'in_progress',now(),now()) returning * into v_attempt;
  elsif v_attempt.status<>'in_progress' then
    update public.practice_assignment_attempts set status='in_progress',practice_session_id=null,
  ...
  return jsonb_build_object('already_completed',false,'attempt_id',v_attempt.id,'assignment_id',v_assignment.id,
    ...
    'available_questions',v_available,'recommended_count',v_count,
...
begin
  ...
  if v_required<1 then raise exception 'No active questions are available for this Practice assignment'; end if;
  ...
  update public.practice_assignment_attempts set status='completed',practice_session_id=v_session.id,
    completed_at=now(),updated_at=now() where id=v_attempt.id returning * into v_attempt;
