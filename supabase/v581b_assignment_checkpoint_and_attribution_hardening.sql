...
--
-- Fixes three related production issues:
...
-- 2) legacy frontend state could submit a genuine Past Paper Practice session as
--    practice_mode='mixed' even though every saved question belonged to one paper;
-- 3) completed assignment checkpoints could remain in cloud storage and be restored
--    even in an incognito browser.
...
begin
  ...
  if v_attempt.id is null then
    ...
    -- V5.8.1B: opening/continuing an existing assignment must not reset its
    -- original started_at boundary. Cross-device checkpoints belong to this
    -- same attempt and remain valid across repeated opens.
...
begin
  ...
  -- Server-derived Past Paper inference. This only upgrades a legacy/mixed
  -- label when every submitted question is Past Paper material from exactly
  -- one year + paper AND the session topic names that same paper.
  select
...
-- Do not let a generic session trigger clear a teacher-assignment checkpoint.
-- Assignment checkpoint cleanup is tied to the assignment attempt completing.
...
-- Re-attribute legacy mixed sessions only when server-side question evidence proves
-- that every saved answer belongs to exactly one Past Paper and the topic names it.
with evidence as (
  select ps.id,
...
-- Recover attempts whose original assignment boundary survives in the checkpoint,
-- but whose started_at was incorrectly reset by a later Continue click. Only link
-- when exactly one full matching session satisfies the assignment evidence.
...
-- Remove any checkpoint left behind for an assignment that is already completed.
delete from public.student_past_paper_checkpoints_v57a c
