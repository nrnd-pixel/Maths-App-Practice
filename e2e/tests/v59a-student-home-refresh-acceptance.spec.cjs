...
async function openStudentHome(page) {
  ...
  await expect(page.locator(PROFILE)).toBeVisible({ timeout: 10_000 });
  return mock;
}
...
test.describe('V5.9A successor — Student Home acceptance contract', () => {
  ...
  test('D — accepted Home owner nodes remain single-source and the V5.8A first-use card is preserved', async ({ page }) => {
    ...
    const before = await page.evaluate(() => ({
      ...
      missionList: document.querySelector('#v572-weekly-missions-card .v572-mission-list')?.textContent || '',
    }));
    ...
    expect(after.missionList).toBe(before.missionList);
  });
