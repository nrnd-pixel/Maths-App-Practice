...
const PILOT = '5e386522-ae0f-4c82-8cf3-6bf0979272f7';
const OTHER_ROSTER = '02a5b3b6-c5d7-4ab4-bd35-2c7989b6b3d1';
...
async function installHarness(page, url, rosterId = PILOT){
  ...
  // Inject after load so config.js's normal staged-script loader does not run.
  await page.addScriptTag({ path: CONFIG_PATH });
}

test.describe('V5.9B Stage 3E deploy-preview smoke selector hard gates', () => {
  test('gate 1 — selector stays absent without flag, outside preview, or for a non-pilot roster', async ({ page }) => {
    ...
    await installHarness(page, `${PREVIEW}/?adaptivePilot=2`, OTHER_ROSTER);
    await page.waitForTimeout(150);
    await expect(page.locator('#v59b2-smoke-selector')).toHaveCount(0);
  });

  test('gate 2 — pilot Q4 launches through ordinary startPractice and restores Learn controls', async ({ page }) => {
