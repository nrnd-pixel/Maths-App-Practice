/* V5.3D1 — Teacher Practice-pool alignment.
   ...
   assignment creation and student assignment targets to the V5.3 unified Practice resource bank. */
(() => {
  'use strict';
  ...
  function topicOptions(cls,strand){
    return [...new Set(matchingQuestions(cls,strand,'').map(question=>trim(question?.topic)).filter(Boolean))]
  ...
  function refreshBuilder(){
    ...
    section.querySelector('.header .tag')?.setAttribute('data-v53d1-aligned','1');
    const badge=section.querySelector('.header .tag');
    if (badge && badge.textContent !== 'V5.3D1') badge.textContent='V5.3D1';
    return true;
  }

  function wireTopicObserver(){
    ...
    if (observedTopicSelect===topicSelect && topicObserver) return true;
    topicObserver?.disconnect();
    observedTopicSelect=topicSelect;
    topicObserver=new MutationObserver(()=>{
      window.setTimeout(()=>refreshBuilder(),0);
    });
    topicObserver.observe(topicSelect,{childList:true});
    return true;
  }
  ...
  function wireUi(){
    ...
    const selectedContent=document.getElementById('selected-class-content');
    if (selectedContent && typeof MutationObserver !== 'undefined'){
      new MutationObserver(()=>{
        refreshUi(false,0);refreshUi(false,100);
      }).observe(selectedContent,{childList:true});
    }
    refreshUi(true,0);
    refreshUi(false,250);
  }
  ...
  const api=Object.freeze({RPC_MAP,routeRpc,logicalQuestionKey,isPracticeEligible,matchingQuestions,availableItems,topicOptions});
