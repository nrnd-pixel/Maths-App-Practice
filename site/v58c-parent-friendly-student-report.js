/* V5.8C — Parent-Friendly Student Report.
   Teacher-only presentation/export layer built entirely from the established
   V50ReportingExport student snapshot. No new analytics request, mastery rule,
   grading logic, persistence, parent account or student-facing data path. */
(() => {
  'use strict';
  ...
  function currentSnapshot(){
    try { return reportingApi()?.buildStudentSnapshot?.() || null; }
    catch { return null; }
  }
  ...
  function strengthTopics(snapshot){
    return recordsOf(snapshot,'topic')
      .filter(row=>statusText(row).includes('secure'))
      .sort((a,b)=>(topicPercent(b)??-1)-(topicPercent(a)??-1) || String(a.topic||'').localeCompare(String(b.topic||'')))
      .slice(0,2);
  }
  ...
  function focusTopics(snapshot){
    return recordsOf(snapshot,'topic')
      ...
      .sort((a,b)=>focusRank(a)-focusRank(b) || (topicPercent(a)??101)-(topicPercent(b)??101) || String(a.topic||'').localeCompare(String(b.topic||'')))
      .slice(0,2);
  }

  function practiceActivities(snapshot){
    return recordsOf(snapshot,'activity')
      .filter(row=>String(row?.activity_mode||'').toLowerCase()==='practice')
      .slice()
      .sort((a,b)=>Date.parse(b.activity_completed_at||0)-Date.parse(a.activity_completed_at||0));
  }
  ...
  function studentYear(snapshot){
    const row=(Array.isArray(snapshot?.records)?snapshot.records:[]).find(item=>item?.student_year!==undefined && item?.student_year!==null && item?.student_year!=='');
