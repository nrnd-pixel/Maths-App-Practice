/* V5.5C — Resume Past Paper Practice.
   Same-device recovery for Past Paper Practice only. Progress is checkpointed
   after a completed logical question. The checkpoint deliberately excludes
   PINs, student access tokens, correct-answer snapshots, hints and explanations.
   Resuming always re-fetches the currently-authorized Practice bank. */
(() => {
  'use strict';

  const ROOT = typeof window !== 'undefined' ? window : globalThis;
  if (ROOT.__v55cResumePastPaperPracticeInstalled) return;
  ...
  async function restoreFromCheckpoint(found){
    ...
    /* Existing Practice retrieval depends on state.year/filter values. Set only
       those query inputs, then re-fetch through the current authorized route. */
  ...
  function installWrappers(){
    ...
    try { ROOT.nextQuestion = nextQuestion; } catch {}
