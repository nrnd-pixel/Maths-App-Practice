...
assert.match(checkpoint,/V5\.8\.1A — Practice cloud result reconciliation/);
assert.match(checkpoint,/Release Audit/);

// Release note accurately consolidates the accepted V5.8 sequence and V5.8.1A maintenance layer.
for (const phrase of [
  ...
  assert(checkpoint.includes(phrase),`Current release note is missing: ${phrase}`);
}

// Accepted runtime chain remains intact; version.js sees V5.8.1A in the same staged list.
for (const loader of [
  './version.js',
...
assert.match(config,/\.\/v58d-content-workflow-consolidation\.js'[\s\S]*\.\/v581a-practice-cloud-result-reconciliation\.js'[\s\S]*\.\/v58-stable-release-checkpoint\.js'/,
  'V5.8.1A must remain loaded before the final release checkpoint.');
...
for (const marker of [
  ...
  '__v581aPracticeCloudResultReconciliationInstalled',
...
assert.match(checkpoint,/\[0,120,420,1100,2200,3400,4600,5200\]/,
  'Current checkpoint refresh must finish after all earlier retained identity bursts.');

// Stable checkpoint remains presentation/audit-only.
assert.doesNotMatch(checkpoint,/MutationObserver/,'Stable checkpoint must not add a DOM observer.');
...
assert.doesNotMatch(checkpoint,/grade_practice_response|request_practice_hint|finalize_exam_attempt|submit_practice_session|save_exam_attempt|get_student_questions|create_teacher_past_paper_assignments|update_teacher_past_paper_assignment|save_question_practice_eligibility|save_topical_exercise_setting/i,
  'Stable checkpoint must not alter grading, retrieval, submission, assignment, content publication or Exam behavior.');
...
console.log('- accepted V5.8A-D plus V5.8.1A runtime chain retained');
console.log('- current identity refresh finishes after earlier retained checkpoint bursts');
