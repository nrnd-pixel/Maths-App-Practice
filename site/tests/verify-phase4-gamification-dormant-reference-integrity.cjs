...
// Check every verifier, not only the gamification-named files. After Checkpoint 2
...
if(unexpected.length){
  ...
  assert.fail(`${unexpected.length} verifier reference(s) still treat or mention a dormant V573/V574 browser file outside the approved integrity guards.`);
}
...
assert.match(checkpoint2,/v573-class-challenges-teacher-gamification\.js/);
assert.match(checkpoint2,/v574-gamification-polish-teacher-controls\.js/);
...
assert.match(v573,/assert\.doesNotMatch\(config,\/\['"\]\\\.\\\/v573-class-challenges-teacher-gamification/,
...
assert.match(v574,/!config\.includes\(["']'\.\/v573-class-challenges-teacher-gamification\.js'["']\)/,
  'V5.7.4 verifier must assert V573 is dormant.');
assert.match(v574,/!config\.includes\(["']'\.\/v574-gamification-polish-teacher-controls\.js'["']\)/,
...
// The exhaustive pass found one later verifier that still treated dormant V573 as
// the owner of the Class Motivation trigger. Keep a direct regression guard here so
// that ownership remains on the consolidated core/teacher modules.
