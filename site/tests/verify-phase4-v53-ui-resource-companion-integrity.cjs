...
const v54a=read('v54a-resource-bank-visibility.js');
const v54b=read('v54b-practice-eligibility-controls.js');
...
// after the engine and before V54A. No historical entry remains active.
...
assert.ok(a<engine && engine<clarityIndex && clarityIndex<v54,'loader order must remain eligibility UI -> Practice engine -> UI/resource clarity -> V54A');
...
assert.ok(v54b.includes("event.target?.closest?.('.tab[data-panel=\"questions-panel\"],#v52b-refresh,.v53a-eligibility-toggle')"),'V54B must continue listening for the exact V53A eligibility-toggle contract');
...
// V53D6 -> V54A is a synchronous API + DOM-node contract, not just presentation.
...
assert.ok(v54a.includes('ROOT.V53D6ResourceBankStatusClarity?.decorate?.()'),'V54A must synchronously invoke V53D6.decorate()');
assert.ok(v54a.includes("meta.querySelector('.v53d6-practice-eligibility-badge')"),'V54A must reuse the exact V53D6 badge');
assert.ok(v54a.includes("if (sourceCategory(row) === 'topical')"),'V54A must not create a fallback topical badge before D6 paints');
