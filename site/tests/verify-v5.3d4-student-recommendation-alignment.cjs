...
const bridge = engine.V53D4StudentRecommendation;
...
assert.match(release,/practice-selection-engine\.js', 'data-practice-selection-engine'/,
  'Consolidated D3/D4 selection engine must remain loaded.');
assert.doesNotMatch(release,/loadScriptOnce\('v53d4-student-recommendation-alignment\.js/,
  'Historical D4 source must remain dormant.');
assert.match(engineText,/get_student_practice_questions_v53d3/,
  'Accepted D3 repeat-aware Practice selection must remain authoritative in the engine.');
...
console.log('V5.3D4 student recommendation alignment verification passed through consolidated engine.');
