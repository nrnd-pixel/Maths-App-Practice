...
// 8) Runtime hardening: deadline monitoring stays class-scoped and event-driven.
...
assert.match(release, /assignment-deadlines\.js/);
...
console.log('V5.1 regression safety verification passed.');
...
console.log('- Deadline reads remain selected-class scoped and event-driven');
console.log('- Student progress uses one active overview while archived V4.9A/V4.9C source remains recoverable');
console.log('- V5.1 content, Question Bank safety, Exam publication, paper-library and resume modules remain loaded');
