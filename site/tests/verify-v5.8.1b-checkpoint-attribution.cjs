...
const sql = fs.readFileSync(
  path.join(repoRoot, 'supabase', 'v581b_assignment_checkpoint_and_attribution_hardening.sql'),
  'utf8',
);
...
expect('create or replace function public.start_student_practice_assignment_v56b(', 'Assignment start/continue RPC successor is missing.');
...
expect('create or replace function public.submit_practice_session_v3(', 'Practice-session submission RPC successor is missing.');
...
expect('coalesce(v_non_past_rows,0)=0', 'Past Paper inference must reject non-past-paper question evidence.');
expect("lower(trim(v_inferred_year::text || ' · ' || v_inferred_paper))", 'Past Paper inference must match the visible paper topic label.');

expect('create or replace function public.clear_completed_past_paper_checkpoint_v57a()', 'Generic Past Paper checkpoint cleanup successor is missing.');
