...
const source = fs.readFileSync(path.join(site,'v56b-teacher-assigned-past-paper-practice.js'),'utf8');
...
new vm.Script(source,{filename:'v56b-teacher-assigned-past-paper-practice.js'});
const api = require(path.join(site,'v56b-teacher-assigned-past-paper-practice.js'));
...
for (const phrase of [
  'Assign Past Paper Practice',
  'Whole selected class',
  'Selected students',
  'Multiple classes',
  'Quick Session',
  'All Available Questions',
  'Teacher Past Paper Assignment',
  'create_teacher_past_paper_assignments_v56b',
  'complete_student_practice_assignment_v56b',
  'mathPastPaperAssignmentV56B'
...
assert.doesNotMatch(source,/exam_attempts|exam_paper_settings|startExam|finishExam/,
  'V5.6B must not modify Exam Mode.');

for (const phrase of [
  "assignment_type text not null default 'targeted'",
  "assignment_type in ('targeted','past_paper')",
  "selection_mode in ('quick','all_available')",
  "strand in ('number','measurement','geometry','statistics','thinking','past_paper')",
  'practice_assignment_attempts_question_target_check',
  'question_target >= 1 and question_target <= 500',
  'create_teacher_past_paper_assignments_v56b',
  'get_student_practice_assignments_v56b',
  'start_student_practice_assignment_v56b',
  'complete_student_practice_assignment_v56b',
  "q.practice_eligible=true",
  "lower(trim(coalesce(q.source_type,'')))='past_paper'",
  "Selected classes must be in the same year level",
  "Every selected student must be active in the selected class",
  "This Practice result is not from the assigned past paper",
  "Finish the full assigned Practice set before completing this assignment"
]) assert(sql.includes(phrase),`V5.6B SQL is missing safety boundary: ${phrase}`);

assert.match(sql,/revoke all on function public\.create_teacher_past_paper_assignments_v56b[\s\S]*from public,anon/i,
  'Teacher assignment creation must not be executable by anon.');
assert.match(sql,/grant execute on function public\.get_student_practice_assignments_v56b\(text\) to anon,authenticated,service_role/i,
  'Student assignment listing must remain available through the token-gated student RPC.');
assert.match(sql,/ps\.practice_mode<>'exam'/,
  'Assignment completion must reject Exam sessions.');
assert.match(sql,/ps\.roster_student_id=v_ticket\.roster_student_id[\s\S]*ps\.class_id=v_ticket\.class_id/,
  'Assignment completion must bind the result to the verified student and class.');

assert.match(config,/\.\/v56a1-bulk-practice-confirmation-bridge\.js'[\s\S]*\.\/v56b-teacher-assigned-past-paper-practice\.js'/,
  'V5.6B must load after the accepted V5.6A stack.');
