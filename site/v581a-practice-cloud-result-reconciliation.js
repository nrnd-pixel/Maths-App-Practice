...
(() => {
  ...
  function installFinishWrapper(){
    ...
    try {
      ...
      /*
        Item 1 — wrap-order check: v40-platform-polish.js stamps __v40PoolWrapped
        on its finishPractice wrapper. If that sentinel is missing the chain is in
        the wrong order (v581a loaded before polish), meaning ticket rotation would
        be skipped. Warn loudly rather than composing silently in the wrong order.
        The install still proceeds because the reconciliation layer is additive and
        safe in either order; the warning surfaces the problem for diagnosis.
      */
      if (!base.__v40PoolWrapped) {
        console.warn(
          'V5.8.1A: finishPractice was not yet wrapped by v40-platform-polish.js ' +
          '(__v40PoolWrapped sentinel absent). Practice ticket rotation may be skipped. ' +
          'Check the load order in v40-release.js.'
        );
      }
