...
/* PR #271 deploy-preview-only adaptive pilot target selector.
   This is a temporary manual smoke-test aid, not a student feature. It is inert
   on production because it requires both a Netlify deploy-preview hostname and
   ?adaptivePilot=2. It delegates to the existing ordinary Practice owner, then
   pins an item already returned by that normal eligible Practice pool. */
(() => {
  const marker='__v59bAdaptivePilotSmokeSelectorInstalled';
  ...
  const targets={
    ...
    q30:{id:'077872ec-2c3c-402f-9c51-491c77500791',label:'2025 P2 Q30',topic:'Percentages'}
  };
  ...
  async function launch(key){
    ...
    try{
      ...
      console.warn('V5.9B deploy-preview smoke selector failed.',error);
  ...
  function install(){
    ...
    panel.innerHTML='<strong style="display:block;margin-bottom:4px">Deploy-preview pilot smoke test</strong><p style="margin:0 0 10px;color:var(--muted,#667085);font-size:12px">Launch an exact pilot target through normal Practice. Preview only.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="outline" data-v59b2-smoke-target="q9b">2025 P1 Q9(b)</button><button type="button" class="outline" data-v59b2-smoke-target="q4">2025 P2 Q4</button><button type="button" class="outline" data-v59b2-smoke-target="q30">2025 P2 Q30</button></div><p id="v59b2-smoke-status" aria-live="polite" style="margin:10px 0 0;font-size:12px"></p>';
