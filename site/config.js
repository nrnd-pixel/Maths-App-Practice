...
/* Stage 3E deploy-preview-only adaptive target selector.
   Temporary manual smoke-test aid only. It is inert unless all three gates hold:
   Netlify deploy-preview hostname, ?adaptivePilot=2, and the sole pilot roster UUID.
   It delegates to ordinary startPractice(), then pins an item already returned by
   the normal eligible Practice pool. No direct Supabase/adaptive RPC is introduced. */
(() => {
  const marker='__v59bAdaptivePilotSmokeSelectorInstalled';
  const pilotRoster='5e386522-ae0f-4c82-8cf3-6bf0979272f7';
  ...
  const isPilot=()=>{
    try{
      return typeof activeStudentAccess!=='undefined'&&
        String(activeStudentAccess?.roster_student_id||'')===pilotRoster;
    }catch{return false;}
  };
  ...
  async function launch(key){
    ...
    if(!isPilot()){status('This smoke selector is available only to the pilot demo account.',true);return;}
