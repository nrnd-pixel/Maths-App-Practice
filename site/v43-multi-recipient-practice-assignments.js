/* V4.3B — Multi-Student & Multi-Class Practice Assignments.
   Supersedes only the staged V4.3A teacher assignment admin UI.
   Student assignment visibility/start/completion remain server-enforced by the V4.3 recipient model. */
(() => {
  'use strict';
  ...
  function sameYearClasses(cls){
    if (!cls || typeof teacherClasses === 'undefined') return [];
  ...
  function populateTopics(){
    ...
    if (availability){
      ...
      availability.textContent = count
        ? `${count} logical question${count===1?'':'s'} available for Year ${Number(cls.year_level)}. Multi-class assignment is limited to this same year level.`
  ...
  function ensureSection(){
    ...
    section.innerHTML = `
      ...
      <div class="v43b-grid">
        ...
        <div class="v43b-wide help" id="v43b-availability"></div>
      </div>
  ...
  function wire(){
    ...
    try {
      if (typeof renderClassAdmin === 'function' && !renderClassAdmin.__v43bWrapped){
        ...
        wrapped.__v43bWrapped = true;
        renderClassAdmin = wrapped;
      }
    } catch (error){
      console.warn('V4.3B could not wrap class renderer.',error);
    }
  }
