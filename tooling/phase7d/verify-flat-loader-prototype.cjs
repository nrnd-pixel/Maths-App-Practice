...
assert.equal(hashObject('site/config.js'),'cc21493af2d53b30b3d25e7bb53d98a24f6e669b','config.js must remain byte-identical');
...
assert.equal(plan.length,94,'prototype must preserve 94 symbolic positions');

const scripts = plan.filter(step => step.kind === 'script');
const boundaries = plan.filter(step => step.kind === 'release-presentation-boundary');
assert.equal(scripts.length,93,'prototype must load 93 scripts because v40-release transport is replaced by one boundary hook');
...
assert.equal(plan.filter(step => step.tier === 1).length,53);
assert.equal(plan.filter(step => step.tier === 2).length,41);
assert.ok(plan.slice(0,53).every(step => step.tier === 1));
assert.ok(plan.slice(53).every(step => step.tier === 2));

const firstTier2 = plan[53];
...
console.log('PASS: Phase 7D-C dormant flat-loader prototype preserves the 53 -> 41 symbolic order, authority predecessors, tier-2 duplicate guards and release-identity boundary without changing production bytes.');
