...
assert.equal(hashObject('site/config.js'),'508341fa4d359135646d615ed9751e709fa6315d','config.js must remain byte-identical');
...
assert.equal(plan.length,86,'prototype must preserve 86 symbolic positions');

const scripts = plan.filter(step => step.kind === 'script');
const boundaries = plan.filter(step => step.kind === 'release-presentation-boundary');
assert.equal(scripts.length,85,'prototype must load 85 scripts because v40-release transport is replaced by one boundary hook');
...
assert.equal(plan.filter(step => step.tier === 1).length,45);
assert.equal(plan.filter(step => step.tier === 2).length,41);
assert.ok(plan.slice(0,45).every(step => step.tier === 1));
assert.ok(plan.slice(45).every(step => step.tier === 2));

const firstTier2 = plan[45];
...
console.log('PASS: Phase 7D-C dormant flat-loader prototype preserves the 45 -> 41 symbolic order, authority predecessors, tier-2 duplicate guards and release-identity boundary without changing production bytes.');
