...
assert.equal(hashObject('site/config.js'),'7307eae1b864bf05778f6daacc7a099b7b563e90','config.js must remain byte-identical');
...
assert.equal(plan.length,90,'prototype must preserve 90 symbolic positions');

const scripts = plan.filter(step => step.kind === 'script');
const boundaries = plan.filter(step => step.kind === 'release-presentation-boundary');
assert.equal(scripts.length,89,'prototype must load 89 scripts because v40-release transport is replaced by one boundary hook');
...
assert.equal(plan.filter(step => step.tier === 1).length,49);
assert.equal(plan.filter(step => step.tier === 2).length,41);
assert.ok(plan.slice(0,49).every(step => step.tier === 1));
assert.ok(plan.slice(49).every(step => step.tier === 2));

const firstTier2 = plan[49];
...
console.log('PASS: Phase 7D-C dormant flat-loader prototype preserves the 49 -> 41 symbolic order, authority predecessors, tier-2 duplicate guards and release-identity boundary without changing production bytes.');
