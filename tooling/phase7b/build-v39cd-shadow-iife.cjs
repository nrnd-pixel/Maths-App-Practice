...
function verifyCandidateOrder() {
  ...
  assert.deepEqual(
    ...
    'V39C/V39D shadow candidates must remain adjacent and in classic-script source order',
  );
  assert.equal(loaded[first - 1], 'v39-practice-polish.js',
    'V39C/V39D must remain immediately after V39B Practice polish');
  assert.equal(loaded[first + SOURCES.length], 'v40-student-platform.js',
    'V39C/V39D must remain immediately before the V40 student platform');
...
async function buildShadowIife() {
  ...
  const result = await build({
    ...
    stdin: {
      ...
      sourcefile: 'v39cd-shadow-entry.js',
    },
    bundle: true,
    format: 'iife',
    platform: 'browser',
  ...
  assert.equal(result.outputFiles.length, 1, 'Expected one in-memory V39C/V39D shadow IIFE');
...
async function main() {
  ...
  const outputPath = path.join(outputDirectory, 'v39cd-dashboard-state-compat.iife.js');
  fs.mkdirSync(outputDirectory, { recursive: true });
  fs.writeFileSync(outputPath, code);
  console.log(`Shadow-only V39C/V39D IIFE: ${path.relative(ROOT, outputPath)} (${Buffer.byteLength(code)} bytes)`);
}
