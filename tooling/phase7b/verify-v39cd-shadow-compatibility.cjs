...
async function main() {
  ...
  assert.doesNotMatch(bundle, forbiddenAuthority,
    'V39C/V39D shadow IIFE contains forbidden network/storage/Practice/Exam/assignment authority');
  ...
  try {
    for (const scenario of ['ready', 'late']) {
      ...
      assert.deepEqual(shadow, classic,
        `${scenario}: V39C/V39D shadow IIFE diverged from classic scripts`);
      console.log(`PASS: ${scenario} DOM — classic V39C/V39D scripts and shadow IIFE are structurally equivalent`);
    }
  } finally {
    await browser.close();
  }

  console.log('PASS: Phase 7B-K V39C/V39D shadow compatibility contract');
}
