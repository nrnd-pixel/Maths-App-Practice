...
function verifyCandidateOrder() {
  ...
  assert.deepEqual(
    ...
    'V58A/V58B shadow candidates must remain adjacent and in classic-script source order',
  );
  assert.equal(loaded[first - 1], 'v576-feedback-presentation-bundle.js',
    'V58A/V58B must remain immediately after the accepted V576 presentation bundle');
  assert.equal(loaded[first + SOURCES.length], 'v58c-parent-summary-presentation-bundle.js',
    'V58A/V58B must remain immediately before the accepted V58C production bundle');
...
async function buildShadowIife() {
  ...
  const result = await build({
    ...
    stdin: {
      ...
      sourcefile: 'v58ab-shadow-entry.js',
    },
    bundle: true,
    format: 'iife',
    platform: 'browser',
  ...
  assert.equal(result.outputFiles.length, 1, 'Expected one in-memory V58A/V58B shadow IIFE');
...
async function main() {
  ...
  const outputPath = path.join(outputDirectory, 'v58ab-first-use-workspace-compat.iife.js');
  fs.mkdirSync(outputDirectory, { recursive: true });
  fs.writeFileSync(outputPath, code);
  console.log(`Shadow-only V58A/V58B IIFE: ${path.relative(ROOT, outputPath)} (${Buffer.byteLength(code)} bytes)`);
}
